   ...
   if (ec_init(ifname))
   {
      ...
      if ( ec_config(FALSE, &IOmap) > 0 )
      {
         ...
         if (ec_slave[0].state == EC_STATE_OPERATIONAL )
         {
            ...
            for(i = 1; i <= 5000; i++)
            {
//                printf("Processdata cycle %5d , Wck %3d, DCtime %12"PRId64", dt %12"PRId64"",
//                   dorun, wkc , ec_DCtime, gl_delta);
//                printf(" %d",(boo)out_EL2004->outvalue1);
//                printf(" %d",(uint8)out_EL2004->outvalue2);
//                printf(" %d",(uint8)out_EL2004->outvalue3);
//                printf(" %d",(uint8)out_EL2004->outvalue4);
//                 in_EL1014 = (in_EL1014t*) ec_slave[2].inputs;
// //                in_EL1014->channel1 = 1;
//                in_ATIAxiaFTSensor = (in_ATIAxiaFTSensort*) ec_slave[5].inputs;
//                out_EL2004 = (out_EL2004t*) ec_slave[3].outputs;
//                out_EL2004->channel1 = 1;
//             printf("%d\n", in_EL1014->channel2);
//             printf("%d\n", in_ATIAxiaFTSensor->Fz);
                in_EL1014 = (in_EL1014t*) ec_slave[2].inputs;
//                in_EL1014->channel1 = 1;
               in_ATIAxiaFTSensor = (in_ATIAxiaFTSensort*) ec_slave[5].inputs;
               out_EL2004 = (out_EL2004t*) ec_slave[3].outputs;
               ...
               if (k < 50) {
                out_EL2004->channel1 = 1;
                out_EL2004->channel3 = 1;
               }
               else {
                   out_EL2004->channel1 = 0;
                out_EL2004->channel3 = 0;
                }
                ec_send_processdata();
               for(j = 0 ; j < oloop; j++)
               {
//                   printf(" %d", *(ec_slave[0].outputs + j));
               }
               for(j = 0 ; j < iloop; j++)
               {
